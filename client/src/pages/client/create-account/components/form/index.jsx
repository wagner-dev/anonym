        ...
        <Body>
            ...
            {Object.keys(errors).length || alert.msg.length ? 
                <Alert type={alert.type || 'err'}>
                    <div>
                        <span>{alert.msg.length ? alert.msg : ( errors.username.type === 'pattern' ? 'Seu nome não deve conter caracteres especiais' : 'Campo(s) inválido(s)' )}</span>
                    </div>
                </Alert>
            :
                null
            ...
            <Form onSubmit={submit(RequestLogin)}>
                <Input
                    ...
                    {...form("username", {required: true, minLength: 2, maxLength: 64, pattern: /^[A-Za-z0-9-_.]+$/img})}
                    type="text"/>
                </Input>
                <Input
                    onFocus={TextTop}
